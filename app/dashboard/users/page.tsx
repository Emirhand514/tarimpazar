...
export default function AdminUsersPage() {
  ...
  const handleBanUser = async () => {
    ...
    if (result.success) {
      ...
      startTransition(async () => { // Re-fetch users to update status
        const fetchedUsers = await fetchUsersAction(searchQuery);
        setUsers(fetchedUsers);
      });
    } else {
  ...
  const handleUnbanUser = async () => {
    ...
    if (result.success) {
      ...
      startTransition(async () => { // Re-fetch users
        const fetchedUsers = await fetchUsersAction(searchQuery);
        setUsers(fetchedUsers);
      });
    } else {
                            ...
                            <TableCell>
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        ...
                                        <DropdownMenuSeparator />
                                        {/* Delete Listing (Admin User cannot delete himself) */}
                                        {user.role !== "ADMIN" && ( // Assuming current admin can't delete self
                                            <DropdownMenuItem className="text-red-600 focus:text-red-600 focus:bg-red-50" onClick={() => { /* Implement delete confirmation */ }}>
